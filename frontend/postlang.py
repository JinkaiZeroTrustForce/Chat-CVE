...
def sendLang():
    ...
    # dockerを使う場合
    url = "http://backend:5173/generate_question"
...
def input():
    ...
    if request.method == "POST":
        ...
        user_answers.append(
            {"where": input1, "why": input2, "level": level, "question": question}
        )
        global_index += 1
        # 模範解答の表示＋「次へ」ボタン
        return render_template(
...
def log():
    ...
    send_json = copy.deepcopy(global_questions)
    for i, user_answer in enumerate(user_answers):
        send_json[i]["user_answer"] = user_answer["where"]
        send_json[i]["user_explanation"] = user_answer["why"]
        
    # path = "http://localhost:5173/evaluation"
    
    # dockerを使う場合
    path = "http://backend:5173/evaluation"

    response = requests.post(path, json=send_json).json()

    return render_template(
        "/log.html",
        global_questions=global_questions,
        evaluation_question=response,
